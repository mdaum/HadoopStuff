...
public class IpStats {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 2) {
      System.err.println("Usage: IpStats <input path> <output path>");
      System.exit(-1);
    }
    //define the job to the JobTracker
    Job job = Job.getInstance();
    job.setJarByClass(IpStats.class);
    job.setJobName("Ip Stats");
    ...
    job.setMapOutputValueClass(ADURecord.class);
    job.setOutputKeyClass(Text.class);
    job.setOutputValueClass(ADURecord.class);
